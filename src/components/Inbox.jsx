        ...
        <div className='flex items-center gap-1'>
          {
            mailType.map((item, index) => {
              return (
                <button key={index} onClick={() => setMailTypeSelected(index)} className={` ${mailTypeSelected === index ? 'border-b-4 border-b-blue-600 text-blue-600' : 'border-b-4 border-b-transparent'} flex item-center gap-2 p-4 w-52 hover:bg-gray-100`}>
                  {item.icon}
                  <span>{item.text}</span>
                </button>
              )
